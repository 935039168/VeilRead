...
- 扩展运行时纯原生 JS：无运行时依赖、无构建步骤、无打包器；开发审计仅使用 `parse5` devDependency
...
| 在线书记入书库 | 以「目录页 URL」为身份（无目录则以章节页），LRU 保留最近 60 本；书库中标记 [在线] 并显示最后章节与进度，续读从 popup、网页阅读层或原生侧边栏进入 |
| 手动添加与编辑 | 设置中心「书库管理」只负责导入、添加在线书、重命名/编辑、重置进度与删除，不直接打开阅读层；书名留空时自动使用网站域名，修改目录/书籍链接会作为新书处理并清除旧进度 |
...
| 自由悬浮窗（默认） | 可拖拽（顶部把手）、四角缩放、大小无上限；位置尺寸自动记忆；可选边缘吸附与鼠标移出自动收起。收起后的恢复圆点会同步到所有已注入的普通 HTTP/HTTPS 页面，点击任一页面的圆点只在该标签页恢复完整窗体；移动、缩放或重置窗口后，圆点重新跟随窗口（已吸附时在对应边缘，未吸附时在右下角） |
...
| 鼠标离开 | 贴边面板启用自动隐藏后会**完全隐藏**，绝不变成自由悬浮圆点；自由悬浮窗可收起为恢复圆点，已吸附时圆点贴在吸附边，未吸附时位于窗口右下角 |
...
自由悬浮窗缩小后，所有已注入页面同步显示恢复圆点；从某一页恢复时，其它页面的圆点会消失，只在当前页打开一个完整阅读窗。切换标签页会默认隐藏旧标签页的完整阅读窗，目标页不会自动展开正文。贴边模式始终保持贴边形态，原生侧边栏模式只使用浏览器侧边栏；浏览器内部页、商店页等禁止注入的页面不参与同步。

### 2.5 外观与设置

- **三种 UI 形态的样式互相独立**：配色方案（纯白/冷灰/米黄/纸白/墨绿/暗夜/纯黑）、文字色、背景色、透明度（1%–100%）、毛玻璃开关，按形态分别保存；其中“冷灰”以近白的冷色背景融入普通网页，适合低调阅读。字体/字号/行距/段首缩进为共用。阅读器内 `Aa` 所作修改会实时同步至设置中心
- 透明度 < 98% 时可选毛玻璃（背景模糊）或纯透明
- 设置中心五个分区（显示方式 / 阅读外观 / 呼出与隐藏 / 站点规则 / 书库管理），每个配置项配非技术向说明；阅读外观带实时预览，书库管理区不提供逐书打开操作
...
│   ├── reader-session.js     跨标签页阅读 UI 纯状态机
...
| 阅读窗瞬时协调状态 | `chrome.storage.session` | 只保存 mode、hidden/panel/bead、标签页/文档标识和 revision；浏览器会话结束后清除，不复制书籍或正文 |
...
**service worker**：事件驱动、空闲即休眠；每次被消息唤醒都从 storage/IDB 重建上下文。同时它是设置和进度写入的单一串行入口，并以 revision 化的 `chrome.storage.session` 状态协调跨标签页恢复圆点和唯一完整窗体，避免多标签页读改写或显示状态互相覆盖。
...
```js
{
  ...
  trigger: {
    ...
    autoHideMode: 'hide' | 'collapse',  // 旧值兼容；贴边模式统一完全隐藏，collapse 仅用于自由悬浮窗
...
开发与发布检查要求 Node.js 20。首次检出或依赖锁文件更新后，先安装锁定的开发依赖：

```text
npm ci
```

`parse5` 仅供发布审计解析站点 HTML，不会进入扩展运行时或发布 ZIP。
...
书籍、进度、设置和站点规则仅存储于浏览器本地的 `chrome.storage.local` 与扩展源 IndexedDB。`chrome.storage.session` 只保存当前浏览器会话内的阅读窗协调元数据，不复制或保存书籍、正文与阅读内容。VeilRead 不建设服务器、不上传数据。唯一的外网流量是用户打开在线阅读后，对当前小说站同源章节和目录的请求；该请求带该站登录态以读取用户本来就能访问的内容，不会自动跟随跨站章节链接。
## 6. 发布与合规
